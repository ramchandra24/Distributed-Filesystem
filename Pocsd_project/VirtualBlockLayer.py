...
class Operations():
    ...
    def get_data_block(self, block_number):
        ...
        #print "Get data from server1 ", (time.time() - start_time)
        ...
        #print "Get data from server1 ", (time.time() - start_time)
        #parity = filesystem.get_data_block(parity_server, parity_block)
        ret_data = list(data1)
        if not data1:
    ...
    def get_valid_data_block(self):
        ...
        print "get new block A:B:P", serverA_number, serverB_number, parity_server_num
        ...
        print "get new block A:B:P", serverA_blknum, serverB_blknum, parity_blknum
        ...
        print "Adding to vlist : ", server_list
        self.print_vnodes()
    ...
    def get_server1_and_block_num(self, block_number):
        print "request block numb", block_number
        self.print_vnodes()
    ...
    def free_data_block(self, vblock_number):
        ...
        if -1 == status:
            print ("Error VirtualBlockLayer: Server " + str(parity_server) + " not responding")
    ...
    def update_data_block(self, vblock_number, block_data):
        ...
        if -1 == status:
            print ("Error VirtualBlockLayer: Server " + str(server2) + " not responding")
        status = filesystem.update_data_block(parity_server, parity_blknum, block_data)
        if -1 == status:
            print ("Error VirtualBlockLayer: Server " + str(server2) + " not responding")
